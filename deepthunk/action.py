...
class ActionLogitSpace:
    ...
    def compile(self):
        ...
        self.logitSpace = LogitSpace()
        self.logitSpace[:self.num_actions] = TokenChoice(self.action_types)

        tidx = self.num_actions
        for type_name, ty in self.types.items():
            lb = tidx
            rb = len(ty)
            self.logitSpace[lb:rb] = ty

    def encode(self, sequence: List[Dict[str, Any]]) -> List[int]:

        tokens = [self.logitSpace[0]["START"]]
        for step in sequence:
